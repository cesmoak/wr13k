...
export const islands = [
  { x: 0, z: 0, rx: 150, rz: 84, height: 20, seed: 431 },
...
// Inner rocks start at index 4: 1529 + 4 * 43 preserves their original seed 1701.
export const reefRocks = [[397,70,12,10],[380,0,13,12],[424,-70,14,11],[391,-140,12,9],
  [291,-81,11,8],[300,-99,9,10],[307,-116,8,7],[322,-189,10,11],[333,-204,9,8],[340,-218,11,10]]
  .map(([x,z,r,height],i)=>({x,z,rx:r,rz:r*.85,height,seed:(i<4?2101:1529)+i*43,rocky:true,reef:true}));
export const outerReefRocks=reefRocks.slice(0,4);
...
// Distance to an island's uneven elliptical shore, in world units.
export function shoreDistance(x,z,land,scale=1){
  const dx=(x-land.x)/land.rx,dz=(z-land.z)/land.rz;
  return (Math.hypot(dx,dz)-shoreRadius(Math.atan2(dz,dx))*scale)*Math.min(land.rx,land.rz);
}

// A shallow sandy shelf slopes continuously away from each generated shore.
export function seabedHeight(x,z) {
  let shore=Infinity;
  for(const island of landforms)shore=Math.min(shore,shoreDistance(x,z,island));
